  ...
  getObjectMetadata(objects) {
    if (objects && objects.length) {
      ...
      return BbPromise.all(headObjectObjects).then((result) => {
        // Add key and directory to results.
  ...
  checkIfDeploymentIsNecessary(objects, funcLastModifiedDate) {
    if (objects && objects.length) {
      ...
      if (this.serverless.service.package.artifact) {
        ...
        // (Manual artifacts _can_ be outputted to the target `.serverless` directory and don't
        // want double-counting which will throw off cache matching.)
      ...
      return BbPromise.all(zipFileHashesPromises).then(zipFileHashes => {
        // TODO(RYAN): HERE -- convert to _object_ of function/layer and hash.
        // TODO(RYAN): NOTE -- the sorted list thing doesn't work anymore.

        console.log('TODO HERE EARLY', {
          zipFilePaths,
          zipFileHashes
        });

        // Create maps of local + remote artifact paths to hashes.
        ...
        const remoteMap = Object.keys(objects).reduce((memo, key) => {
          // Normalize file path to after last slash per other logic in this file.
          const filePath = objects[key].Key.substring(objects[key].Key.lastIndexOf('/') + 1);
          memo[filePath] = objects[key].Metadata.filesha256 || '';
        ...
        console.log('TODO HERE normCfTemplate', JSON.stringify(normCfTemplate, null, 2));

        remoteHashes.sort();
        localHashes.sort();
        console.log('TODO HERE -- THE CHECK', JSON.stringify({
          changedAfterDeploy,
          remoteHashes,
          localHashes,
          isEqual: _.isEqual(remoteHashes.sort(), localHashes.sort())
        }, null, 2));
