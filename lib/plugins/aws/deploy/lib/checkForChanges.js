  ...
  checkIfDeploymentIsNecessary(objects, funcLastModifiedDate) {
    if (objects && objects.length) {
      ...
      return BbPromise.all(zipFileHashesPromises).then(zipFileHashes => {
        ...
        const cfTemplateName = this.provider.naming.getCompiledTemplateS3Suffix();
        ...
        console.log("TODO HERE CF TEMPLATE", JSON.stringify(normCfTemplate, null, 2));
        ...
        if (!changedAfterDeploy) {
          ...
          } else if (localMap[cfTemplateName] === remoteMap[cfTemplateName]) {
            // If the CF template hasn't changed, then check if only some
            // functions / layers have changed and mutate the CF template to
            // reuse existing deployed ones.

            // TODO: HERE -- Mutate CF template up way higher???

            this.serverless.service.getAllLayers().forEach((layerName) => {
              const layerZip = `${layerName}.zip`;
              if (localMap[layerZip] === remoteMap[layerZip]) {
                // TODO: IMPLEMENT MUTATION.
                console.log("TODO MUTATE LAYER", JSON.stringify({
                  layerName,
                  layerZip
                }));
              }
